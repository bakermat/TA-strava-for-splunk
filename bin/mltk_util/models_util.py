...
def get_model_list_by_experiment(rest_proxy, namespace, experiment_id):
    url_params = [
        ('search', '__mlspl__exp_*{}*{}'.format(experiment_id, MODEL_EXTENSION)),
        ('count', '0'),
    ]
    url = rest_url_util.make_get_lookup_url(
        rest_proxy, namespace=namespace, url_params=url_params
    )
...
def model_name_to_filename(name, tmp=False):
    ...
    return '__mlspl_' + name + MODEL_EXTENSION + suffix
...
def update_model_file_from_rest(model_filename, searchinfo, namespace, model_filepath):
    ...
    url = rest_url_util.make_lookup_url(
        rest_proxy, namespace=namespace, lookup_file=model_filename
    )
...
def create_model_file_from_rest(model_filename, searchinfo, namespace, model_filepath):
    ...
    payload = {'eai:data': model_filepath, 'name': model_filename, 'output_mode': 'json'}
...
def move_model_file_from_staging(model_filename, searchinfo, namespace, model_filepath):
    ...
    if not reply['success']:
        if reply['error_type'] == 'ResourceNotFound':
            reply = create_model_file_from_rest(
                model_filename, searchinfo, namespace, model_filepath
            )
...
def copy_model_to_staging(src_model_name, searchinfo, dest_dir_path=None):
    ...
    try:
        ...
        logger.debug(str(e))
        raise Exception(
            'Failed to disk copy model %s to the staging area.' % src_model_filepath
        )
