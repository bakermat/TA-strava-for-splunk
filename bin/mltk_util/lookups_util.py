...
def build_lookups_query_params(query_params, username):
    ...
    user_filter = (
        '((eai:acl.sharing="user" AND eai:acl.owner=%s) OR (eai:acl.sharing!="user"))'
        % escaped_username
    )
...
def get_lookups_from_splunk(searchinfo, namespace, cb_reply_parser, query_params):
    ...
    url = make_get_lookup_url(
        rest_proxy, namespace=namespace, lookup_file=None, url_params=query_params_copy
    )
...
def lookup_name_to_path_from_splunk(
    lookup_name, file_name, searchinfo, namespace=None, lookup_type=None
):
    ...
    if should_use_btool(searchinfo):
        ...
        if is_valid:
            ...
            file_path = lookup_name_to_path_distributed(
        ...
        file_path = lookup_name_to_path_from_splunk_rest(
            file_name, searchinfo, namespace, lookup_type
        )
...
def lookup_name_to_path_from_splunk_rest(
    file_name, searchinfo, namespace=None, lookup_type=None
):
...
def lookup_name_to_path_distributed(lookup_name, file_name, searchinfo, namespace, lookup_type):
    ...
    required_searchinfo_fields = ('app', 'username')
    ...
    if namespace == 'user':
        results[user] = btool_util.get_lookups_btool(
            user=user, app=app, target_dir=bundle, lookup_type=lookup_type
        )
    for role in roles:
        results[role] = btool_util.get_lookups_btool(
            user=role, app=app, target_dir=bundle, lookup_type=lookup_type
        )
...
def get_lookup_from_btool_result(
    btool_dict, lookup_name, file_name, user, app, roles, namespace
):
    try:
