...
class RestHandler(PersistentServerConnectionApplication):
    ...
    def handle(self, in_string):
        ...
        try:
            ...
            if not is_valid_identifier(handler_name):
                raise Exception(
                    'REST handlers can only start with letters and underscores and can contain only letters, numbers, and underscores'
                )
            ...
            try:
                ...
                logger.exception(e)
                return {'payload': 'Unknown REST endpoint: %s' % handler_name, 'status': 404}
            try:
    ...
    def get_path_info_parts(request):
        try:
            ...
            return list(filter(bool, request['path_info'].split('/')))
        except KeyError:
            return []
