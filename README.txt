...
#### (2) Once setup:
- Go to <https://www.strava.com/oauth/authorize?client_id=[client_id]&redirect_uri=http://localhost&response_type=code&scope=activity:read_all,profile:read_all>, make sure you replace [client_id] with your Client ID.
...
### Troubleshooting
- Logs are in $SPLUNK_HOME/var/log/splunk/ta_strava_for_splunk_strava_api.log, or search for "index=_internal source=/opt/splunk/var/log/splunk/ta_strava_for_splunk_strava_api.log".
- Most common issue is the access code being invalid or expired. If that's the case, go to step 2 to get a new access code.
- If you want to reindex all activities, either delete the input and create a new one with a new name or use the 'Reindex data' option (since 2.5.2) to retrieve data since the specified timestamp.
...
### Release Notes
v3.0.0
- Added support for activity streams, which allow for vendor-agnostic second-by-second analysis of all sensor data in an activity (time, distance, heart rate, power, altitude and more) including getting it from activities that are already in Splunk so no need to reindex data. *This feature requires the Python Scientific Computing Add-On for Splunk in order to parse the data, and has only been tested on Linux at this stage.*
